...
2026-07-31 对全部 25 个历史 `stage2_fullflow_*` rosbag 重新执行严格审计后，只有
`1350`、`1500` 两轮同时满足完整 11 相位、无 PX4 failsafe、最终 landed 和有效投放
证据。共有 8 次真实投放，其中 7 次在 0.2 m 内，当前统计为 **87.5%**。`1832` 失败轮
已把连续成功计数重置为 0，因此仍需重新完成 10 次连续完整成功；新增投放前两次均命中
即可先达到 9/10=90%，正式执行仍以新增 10 次全部命中为目标。严格审计会按 bag 名时间
排序并只计算末尾未中断成功串，报告为 `/tmp/stage2_history_audit_v2.json`。

首次启用活动轨迹障碍抢占后的 `1832` 回归在 EGO_TRANSIT 安全停止。排查确认根因不是
真实障碍或膨胀半径，而是 RTX LiDAR 的实体姿态与 ROS 静态 TF 不一致：USD 传感器采用
`+90 deg X`，旧 TF 却发布成 `+90 deg Z`，导致点云被错误旋转并在投放区形成假占用。
场景配置现以同一 `LIDAR_ORIENTATION_WXYZ` 同时生成 USD 四元数和 ROS `xyzw` TF；冷启动
后实测 `avoidance_base_link -> avoidance_lidar` 为 `x=0.7071,w=0.7071`，20 帧点云中
粗目标 `[5.5,-3.3,1.8]` 的最近障碍距离为 1.480 m。粗目标仍保留在成功基线对应的
`[5.5,-3.3,1.8]`，由下视视觉完成到投放圆心 `[5.5,-3.5]` 的末段对准。

2026-08-01 冷启动实飞门禁包 `bags/stage2_coarse_target_tffix_20260801_0310` 已验证新粗
目标：`EGO_TRANSIT` 后在 788.707 s 进入 `TARGET_SEARCH`，50 ms 后进入
`VISUAL_ALIGN`，当时真值约为 `(5.426,-3.113,1.807) m`。验证脚本随后主动发送 LAND，
PX4 正常判地并解除武装，无 failsafe；该包只作为粗目标门禁，不计入正式 10 次连续
回归。`1832` 仍作为失败回归保留，正式连续计数从 0 重新开始。

同日首次正式回归在投放与返航通道点均成功后，近场返架暴露高度目标抖振：旧
`stagedReturnRawTarget()` 在水平误差 2 cm 内外把 Z 目标在 1.185 m 和 1.80 m 之间
直接切换。实时探针记录到误差从 1.98 cm 增到 2.06 cm 时，垂直指令由
`-0.05 m/s` 翻为 `+0.05 m/s`，导致重复升降，无法可靠同时满足 8 mm/0.05 m/s
Land 门。该轮通过 9 mm（仍在 10 mm 物理间隙内）的操作员安全门发送公开 LAND，
最终 `(4.559,-0.373,1.130) m`、`landed=true/armed=false/failsafe=false`；诊断包为
`bags/stage2_return_chatter_20260801_0338`，不计正式回归。

修复采用单向下降锁存：首次进入 2 cm 圈后保持 approach 高度目标，短暂横向越界和
HOLD→RETURN 均不再重新爬升；仅在新任务 PREFLIGHT 或 DROP_HOLD→RETURN 初始边沿
复位。新增 `ReturnGuidance.DescentLatchNeverRearmsTransitAltitudeDuringReturn`，当前
62 项 C++ 核心测试全部通过。冷启动实飞包 `bags/stage2_fullflow_20260801_0446`
随后关闭该票据：下降中水平误差在 6.2--11.7 mm 波动时，垂直指令始终保持
`-0.05 m/s` 而未反向爬升；最终完整 11 相位、自动 Land/解除武装、无 failsafe，
投放误差 2.28 cm。严格审计本轮 `passed=true`，新的连续计数为 **1/10**。
全屏录屏同时改用 T4 `h264_nvenc`，避免 `libx264` 约 226% CPU 占用加重传感器陈旧
HOLD；原始连续视频为 `videos_raw/20260801_045136-预选赛加分1-全流程_raw.mp4`
（1920×1080/30 fps，2022 s）。2026-08-02 已从该单一连续原片生成 12×等速压缩、
无剪切/无拼接的正式版本 `videos/预选赛加分1-全流程.mp4`：168.5 s、
1920×1080/30 fps、H.264，左上角常驻标明“12×加速｜全程连续无剪辑”，满足不超过
3 分钟要求；SHA-256 为
`751abda75fbfe6919a305b964647ef2d4f2568aab71cc9fdcb39074324789e24`。

第二轮冷启动包 `bags/stage2_fullflow_20260801_0537` 再次自动进入
`TARGET_SEARCH/VISUAL_ALIGN`，投放后经返航通道点和下降锁存完成精准返架；最终
`(4.5588,-0.3866,1.1300) m`、`landed=true/armed=false/failsafe=false`。严格审计
完整 11 相位、载荷静止，投放误差 **0.915 cm**，本轮 `passed=true`。正式连续计数
现为 **2/10**；两次新回归均命中，结合此前 7/8，累计投放为 **9/10=90%**。单次
回归仍不能关闭 10 连续门，且本轮真值速度峰值 `0.607 m/s` 仍需纳入安全包线收口。

第三轮冷启动包 `bags/stage2_fullflow_20260801_0618` 同样严格通过：完整 11 相位、
投放误差 **1.090 cm**、最终 `landed=true/armed=false/failsafe=false`。正式连续计数
更新为 **3/10**；三次新回归全部命中，结合此前 7/8，累计投放为
**10/11=90.9%**。本轮真值速度峰值为 `0.648 m/s`，未关闭速度包线开放项。
...
- `drop_search_pose`：当前 X1 粗搜索位姿为 `[5.5,-3.3,1.8]`；投放圆心仍为
  `[5.5,-3.5]`，最后 0.2 m 只由下视视觉完成。
...
- 通过新出现障碍的动态换轨回归：显式 `PREEMPTION_OBSTACLE` 和
  `ACTIVE_OBSTACLE_REPLAN` 已实现并在隔离 ROS 运行时通过合成点云抢占测试（轨迹 1
  被轨迹 2 抢占，证据 `/tmp/stage2_dynamic_preemption.json`）。Isaac 5.1 Full Kit
  实体障碍测试仍失败关闭：RTX LiDAR 日志报告 MotionBVH 未启用，严格探针在实体生成
  前后只看到约 254/255 点的噪声差，不能把合成点云票据冒充物理动态障碍通过。
